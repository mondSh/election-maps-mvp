...
import maplibregl, { Map as MlMap, type StyleSpecification, type ExpressionSpecification } from "maplibre-gl";
...
export default function MapView({ parties, settlements, points, colorMode, selected, onSelect }: Props) {
  ...
  // ---- recolor when colorMode changes ----
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !map.getLayer("settle-fill")) return;
    map.setPaintProperty("settle-fill", "fill-color", fillExpression(colorMode, parties));
  }, [colorMode, parties]);
